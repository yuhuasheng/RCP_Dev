package com.foxconn.mechanism.dgkpi.handler;
...
import com.foxconn.mechanism.dgkpi.dialog.SelectPdfDialog;
...
public class ImportPdfHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent arg0) throws ExecutionException {	
			...
			if(!("DocumentRevision".equalsIgnoreCase(itemRevision.getTypeObject().getName()))) {
				MessageBox.post(AIFUtility.getActiveDesktop(), reg.getString("error.select"), "Error", MessageBox.ERROR);
				return null;
			}
			Shell shell = app.getDesktop().getShell();
			new SelectPdfDialog(app,shell,reg,(TCComponentItemRevision)aifCom);
