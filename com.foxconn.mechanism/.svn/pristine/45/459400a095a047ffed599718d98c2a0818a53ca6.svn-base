...
public class PRTHHPNMaterialApplyDialog extends Dialog {
	...
	private PRTEditingSupport prtEditingSupport = null;	
	...
	private Label addAndModifyLabel = null; // 新增
	...
	private static final String[] treeTitles = new String[] { "MODEL_NAME", "鴻海料號", "鴻海料號狀態","客戶料號", "中文描述", "英文描述", 
												"數量", "客戶圖號",	"客戶3D版本", "客戶2D版本", "客戶資訊", "下料尺寸", "專案", "模組", "材質", "顏色", "表面處理"};
	
	public PRTHHPNMaterialApplyDialog(AbstractPSEApplication app, Shell parent) {
		super(parent);
		this.app = app;
		...
		System.out.println("==>> partType: " + ItemRevEnum.CommonPartRev.type());
		...
		this.shell = parent;	
		try {
			LoopProgerssDialog loopProgerssDialog = new LoopProgerssDialog(shell, null, reg.getString("progressDialog1.TITLE"));
			loopProgerssDialog.run(true, new IProgressDialogRunnable() {
				...
				public void run(BooleanFlag stopFlag) {
					try {
						...
						topBomInfo = BOMTreeTools.traversalBOMTree(app, session, BUConstant.PRTBUNAME, stopFlag); // 遍历BOM结构树
	...
	private void initUI() {
		...
		fastBtnComposite.setLayout(new GridLayout(5, false));
...
//		functionBtnData.horizontalAlignment = SWT.BEGINNING;
		...
		GridLayout tipGridLayout = new GridLayout(8, false);
...
//		tipData.horizontalAlignment = SWT.END;
		...
		tipData = new GridData(GridData.HORIZONTAL_ALIGN_FILL);
		tipData.horizontalAlignment = SWT.BEGINNING;
		tipData.horizontalSpan = 8; // 水平抢占两列空间
		...
		checkedTreeViewer.setLabelProvider(new PRTCustTreeViewerTableLabelProvider());
		...
		buttonSelectionListener = new ButtonSelectionListener(app, checkedTreeViewer, shell, session, topBomInfo, tipLabel, BUConstant.PRTBUNAME, reg, bomTreeValidation, treeTools);
...
//		buttonSelectionListener.resetTreeColumn(); // 重新设置各列宽，让其文本可以完全显示
	...
	private void createTreeColumn(TreeViewer tv) {
		for (int index = 0; index < treeTitles.length; index++) {
			...
			if (index == 0) {
				columnWidth = 400;
			} else if (index == 1 || index == 2 || index == 3 || index == 4 || index == 5 || index == 7 					
					|| index == 8 || index == 9) {
				columnWidth = 250;
			} else if (index == 10 || index == 12 || index == 13) {
				columnWidth = 350;
			} 
			else {
				columnWidth = 130;
			...
			prtEditingSupport = new PRTEditingSupport(tv, index, treeTools);
			tvc.setEditingSupport(prtEditingSupport);
		}
	}
	 ...
	 * @param topBomInfo
	 */
	private void addPart(BOMInfo topBomInfo) {
		...
		if (objectType.contains(ItemRevEnum.DesignRev.type())) { // 判断且对象类型为设计对象
			...
			child.add(childInfo);	
			childInfo.setParent(topBomInfo);
		}
