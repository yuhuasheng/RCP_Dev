package com.foxconn.mechanism.dgkpi.dialog;
...
import com.foxconn.mechanism.util.PDFUtils;
...
public class SelectPdfDialog extends Dialog{
	...
	public SelectPdfDialog (AbstractAIFUIApplication app, Shell parent, Registry reg,TCComponentItemRevision  itemRevision ) {
		super(parent);
		this.app = app;
			...
			public void widgetSelected(SelectionEvent e) {
			  try {
				   ...
				   if(strText==null||"".equalsIgnoreCase(strText)) {
					...
					// TODO 具体业务逻辑
					System.out.print("path : " + strText);
					Set<String> keySet = CollUtil.newHashSet("Customer PN","HH PN","Vendor PN","Function");
					InputStream in = new FileInputStream(strText);
					Map<String, String> map = PDFUtils.readTable(in, keySet);
					if(CollUtil.isEmpty(map)) {
						map = PDFUtils.readText(strText, keySet);
					}
					if(CollUtil.isEmpty(map)) {
						throw new Exception("未找到Feature");
					}
