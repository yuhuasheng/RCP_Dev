...
public class CustomMaterialBatchImportAction {
	...
	static TCComponentProject project;
	
	public CustomMaterialBatchImportAction() {}
	
	public CustomMaterialBatchImportAction(CustomMateriaBatchImportType customMateriaBatchImportType, TCComponent tcTargetComp, String fileName, ICustomMaterialBatchImportLog log,String actualUser,TCComponentProject project) {
		_blnErrFlag = false;
					...
					for (Map<String, Map<String, String>> entityContentMap : value) {
						...
						if ("79-PCB ASS'Y".equals(key)) {														
							...
							if (!"INCLUDE (SMD/AI/OTHER/PRE-FORMING)".equals(d9ProductTypeL6Str) && !"IC+FW".equals(d9ProductTypeL6Str)) {
								...
								TCComponentFolder selfPartFolder = null;
								Map<String, String> finishModelNameMap = entityContentMap.getOrDefault("成品Model Name", null);
								String finishModelName = finishModelNameMap.getOrDefault("value", null);	
								try {
									TCComponent[] queryResult = TCUtil.executeQuery(_tcTargetComp.getSession(), "__D9_Find_Project_Folder", new String[] {"d9_SPAS_ID"}, new String[] {project.getProjectID()});
									if(queryResult.length>0) {
										TCComponentFolder projectFolder = (TCComponentFolder) queryResult[0];
										selfPartFolder = getSelfPartFolder(projectFolder);
									}
								} catch (Exception e) {
								...
								if (TCUtil.isNull(d9DerivativeDifferentialCodeStr)){																									
									if(selfPartFolder!=null) {
										boolean found = isExsitFinishModelNameFolder(selfPartFolder, finishModelName);									
										if(found) {
											blnIsPass = false;
								...
								boolean found = isExsitFinishModelNameFolder(selfPartFolder, finishModelName+"_"+d9DerivativeDifferentialCodeStr);
								if(found) {
									blnIsPass = false;
									errorMap = entityContentMap.get("衍生機種區別碼");
									errorMap.put("error", "衍生機種區別碼文件夹已存在，请重新输入");
								}
							}
						}
...
//							}														
	...
	//						getSupplierInfo(tcPropValueMap);
							...
							TCComponentItem createdCom = (TCComponentItem) createCustomMaterial(_tcTargetComp, tcType, 1 == itemIdLst.size() ? itemIdLst.get(0) : "", tcPropValueMap);
							if (createdCom!=null) {
								// 指派專案
								try {
									project.assignToProject(new TCComponent[]{createdCom.getLatestItemRevision()});
								} catch (TCException e) {
									// TODO Auto-generated catch block
									e.printStackTrace();
								}
...
public static TCComponent createCustomMaterial(TCComponent tcTargetComp, String tcType, String itemId, Map<String, Object> tcPropValueMap) {
		...
		try {
			...
			if (tcTargetComp != null && com != null && tcTargetComp instanceof TCComponentFolder) {
	...
	public static TCComponent createCustomMaterial2(TCComponent tcTargetComp, String tcType, String itemId, Map<String, Object> tcPropValueMap) throws TCException {
	...
	//获取自编料号文件夹
	public static TCComponentFolder getSelfPartFolder(TCComponent projectFolder) {
		try {
	...
	//获取成品ModelName文件夹
	public static boolean isExsitFinishModelNameFolder(TCComponent selfPartFolder,String finishModelName) {
		try {
