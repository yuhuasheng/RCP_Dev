...
import com.foxconn.mechanism.hhpnmaterialapply.domain.TreeItemUpdateInfo;
import com.foxconn.mechanism.hhpnmaterialapply.util.TreeTools;
import com.foxconn.mechanism.util.CommonTools;

public class DTSAEditingSupport extends EditingSupport {
	...
	public DTSAEditingSupport(ColumnViewer viewer, int index, TreeTools treeTools) {
		super(viewer);
...
//		TreeItem topItem = tv.getTree().getTopItem();
		...
		// 判断顶层BOMLine是否和currentTreeItem的HHPN相同
		...
		// 更新属性值到Tree绑定的实体类中
//		DTSATreePropertiesTools.updateProperties(currentPropertiesInfo, index, value);
		// 同步结构树中的模型数据
		treeTools.syncTreeModelData(topBomInfo, currentInfo, index, value, BUConstant.DTSABUNAME);
		// 刷新结构树中类型为料号对象的属性
