package com.foxconn.mechanism.hhpnmaterialapply.PRT;


import com.foxconn.mechanism.hhpnmaterialapply.domain.BOMInfo;
import com.foxconn.mechanism.hhpnmaterialapply.domain.PropertiesInfo;
import com.foxconn.mechanism.util.CommonTools;

public class PRTTreePropertiesTools {	
	
	/**
	 * 设置属性值
	 * @param propertiesInfo
	 * @param index
	 * @param str
	 */
	public static void updateProperties(PropertiesInfo propertiesInfo, int index, String str) {
		switch (index) {
		case 3:
			propertiesInfo.setCustomerPN(str);
			break;
		case 4:
			propertiesInfo.setChineseDescription(str);
			break;
		case 5:
			propertiesInfo.setEnglishDescription(str);
			break;
		case 7:
			propertiesInfo.setCustomerDrawingNumber(str);
			break;
		case 8:
			propertiesInfo.setCustomer3DRev(str);
			break;
		case 9:
			propertiesInfo.setCustomer2DRev(str);
			break;
		case 10:
			propertiesInfo.setCustomerID(str);
			break;
		case 11:
			propertiesInfo.setSmPartMaterialDimension(str);
			break;
		case 12:
			propertiesInfo.setProjectName(str);
			break;
		case 13:
			propertiesInfo.setModule(str);
			break;
		case 14:
			propertiesInfo.setMaterial(str);
			break;
		case 15:
			propertiesInfo.setColor(str);
			break;
		case 16:
			propertiesInfo.setSurfaceFinished(str);
			break;
		default:
			return;
		}
	}
	
	/**
	 * 判断属性值是否发生更改
	 * @param currentBomInfo
	 * @param index
	 * @param value
	 * @return
	 */
	public static boolean checkModify(BOMInfo currentBomInfo, int index, String value) {
		if (CommonTools.isEmpty(currentBomInfo)) {
			return false;
		}
		if (CommonTools.isEmpty(value)) { // 判断是否为空
			return false;
		}
		PropertiesInfo currentPropertiesInfo = currentBomInfo.getPropertiesInfo();
		switch (index) {
		case 3:
			if (value.equals(currentPropertiesInfo.getCustomerPN())) {
				return false;
			}
			return true;
		case 4:
			if (value.equals(currentPropertiesInfo.getChineseDescription())) {
				return false;
			}
			return true;
		case 5:
			if (value.equals(currentPropertiesInfo.getEnglishDescription())) {
				return false;
			}
			return true;
		case 7:
			if (value.equals(currentPropertiesInfo.getCustomerDrawingNumber())) {
				return false;
			}
			return true;
		case 8:
			if (value.equals(currentPropertiesInfo.getCustomer3DRev())) {
				return false;
			}
			return true;
		case 9:
			if (value.equals(currentPropertiesInfo.getCustomer2DRev())) {
				return false;
			}
			return true;
		case 10:
			if (value.equals(currentPropertiesInfo.getCustomerID())) {
				return false;
			}
			return true;
		case 11:
			if (value.equals(currentPropertiesInfo.getSmPartMaterialDimension())) {
				return false;
			}
			return true;
		case 12:
			if (value.equals(currentPropertiesInfo.getProjectName())) {
				return false;
			}
			return true;
		case 13:
			if (value.equals(currentPropertiesInfo.getModule())) {
				return false;
			}
			return true;
		case 14:
			if (value.equals(currentPropertiesInfo.getMaterial())) {
				return false;
			}
			return true;
		case 15:
			if (value.equals(currentPropertiesInfo.getColor())) {
				return false;
			}
			return true;
		case 16:
			if (value.equals(currentPropertiesInfo.getSurfaceFinished())) {
				return false;
			}
			return true;
		default:
			return false;
		}
	}
}
