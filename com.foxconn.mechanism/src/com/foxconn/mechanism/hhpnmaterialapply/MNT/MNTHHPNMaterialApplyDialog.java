...
import com.foxconn.mechanism.hhpnmaterialapply.MNT.MNTCustTreeViewerTableLabelProvider;
...
public class MNTHHPNMaterialApplyDialog extends Dialog {
	...
	private MNTEditingSupport mntEditingSupport = null;	
	...
	private static final String[] treeTitles = new String[] { "MODEL_NAME", "鴻海料號", "鴻海料號狀態", "中文描述", "英文描述",
			"用量", "材質", "產品重量(g)", "顏色", "備註", "表面處理", "參考尺寸", "水口重量(g/個)", "總重(g/個)" };

	public MNTHHPNMaterialApplyDialog(AbstractPSEApplication app, Shell parent) {
		super(parent);
		this.app = app;
			...
			loopProgerssDialog.run(true, new IProgressDialogRunnable() {
				...
				public void run(BooleanFlag stopFlag) {
					try {
						...
						topBomInfo = BOMTreeTools.traversalBOMTree(app, session, BUConstant.MNTBUNAME, stopFlag); // 遍历BOM结构树
...
//		tipData.horizontalAlignment = SWT.END;
		...
		checkedTreeViewer.setLabelProvider(new MNTCustTreeViewerTableLabelProvider());
		...
		buttonSelectionListener = new ButtonSelectionListener(app, checkedTreeViewer, shell, session, topBomInfo, tipLabel, BUConstant.MNTBUNAME, reg, bomTreeValidation, treeTools);
...
//		buttonSelectionListener.resetTreeColumn(); // 重新设置各列宽，让其文本可以完全显示
	...
	private void createTreeColumn(TreeViewer tv) {
		for (int index = 0; index < treeTitles.length; index++) {
			...
			if (index == 0) {
				columnWidth = 400;
			} else if (index == 1 || index == 2 || index == 3 || index == 4 || index == 6 || index == 9 
					|| index == 10 || index == 11) {
				columnWidth = 200;
			} else {
				columnWidth = 130;
			...
			mntEditingSupport = new MNTEditingSupport(tv, index, treeTools);
			tvc.setEditingSupport(mntEditingSupport);
		}
	}
