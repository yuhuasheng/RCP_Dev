...
public class MntL10EBomDialog extends Dialog {
    private static final double RATIO = 0.8D;
    private Browser             browser;

    public MntL10EBomDialog(AbstractAIFUIApplication app, Shell parent, String url, ResourceBundle reg)
    {
        super(parent);
        Dimension dim = app.getDesktop().getSize();
        Shell shell = new Shell(app.getDesktop().getShell(), SWT.DIALOG_TRIM | SWT.CLOSE | SWT.MAX | SWT.RESIZE);
        shell.setSize((int) (dim.getWidth() * RATIO), (int) (dim.getHeight() * RATIO));
        shell.setText(reg.getString("L10EBOMDialog.TITLE"));
