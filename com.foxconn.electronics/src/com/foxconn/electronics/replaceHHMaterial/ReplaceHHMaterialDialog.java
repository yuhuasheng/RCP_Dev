...
public class ReplaceHHMaterialDialog extends Dialog
{
    private static final double RATIO = 0.8D;
    private Browser             browser;

    public ReplaceHHMaterialDialog(AbstractPSEApplication app, Shell parent, Registry reg, String url)
    {
        super(parent);
        Dimension dim = app.getDesktop().getSize();
        Shell shell = new Shell(app.getDesktop().getShell(), SWT.DIALOG_TRIM | SWT.CLOSE | SWT.MAX | SWT.MIN | SWT.PRIMARY_MODAL | SWT.RESIZE);
        shell.setSize((int) (dim.getWidth() * RATIO), (int) (dim.getHeight() * RATIO));
        shell.setText("批量替换鸿海料号");
