...
import com.foxconn.electronics.login.MyHomeSectionComponent;
...
public class CreateIssueService {
	...
	public CreateIssueService(TCSession session, ResourceBundle reg,String customer,CreateIssueDialog dialog) {
		super();
	...
	private void setTestUser(TCComponentItemRevision itemRevision) throws Exception {
		//  獲取實際用戶
		TCComponent[] executeQuery = TCUtil.executeQuery(session, "__D9_Find_Actual_User", new String[] {"item_id"},
				new String[] {UserLoginSecond.getOSSUserInfo().getEmp_no()});
		if (executeQuery != null && executeQuery.length > 0) {
			String uid = executeQuery[0].getUid();
			setWorker(itemRevision, uid, "d9_IssueTester_ActualUser");
		}
		setWorker(itemRevision, session.getUser().getUid(), "D9_IssueTester");
	}
	
	
	public AjaxResult createIssue(HttpRequest req) {
		String body = req.getBody();
		JSONObject json = JSONUtil.parseObj(body);
		String itemUid = json.getStr("itemUid");
		TCComponent tcComponent = TCUtil.loadObjectByUid(itemUid);
	...
	public AjaxResult createHpIssue(HttpRequest req) {
		...
		try {
			...
			String projectId = json.getStr("tcProject");
			Boolean submit = json.getBool("submit");
			syncDeal(projectId, itemRevision, createItem, submit);
		}catch (Exception e) {
			if(e instanceof TCException) {
				return AjaxResult.error(AjaxResult.STATUS_NO_RESULT, ((TCException) e).getMessage());
			}
	...
	public AjaxResult createDellIssue(HttpRequest req) {
		...
		try {
			...
			if(StrUtil.isNotBlank(json.getStr("subClassify"))) {
				itemRevision.setProperty("d9_IRComponent",json.getStr("subClassify"));
			}
			...
			if(StrUtil.isNotBlank(json.getStr("affectedItems"))) {
				String str = json.getStr("affectedItems");
				List<String> split = StrSplitter.split(str, ",",true,true);
				TCProperty tcProperty = itemRevision.getTCProperty("d9_IRAffectedItemsDell");
				tcProperty.setStringValueArray(split.toArray(new String[0]));
			}
	...
	public void syncDeal(String projectId,TCComponentItemRevision itemRevision,TCComponentItem createItem,Boolean submit) {
		try {
			// 指派專案
			...
			TCUtil.warningMsgBox(e.getMessage(), reg.getString("ERROR.MSG"));
		}
	}
