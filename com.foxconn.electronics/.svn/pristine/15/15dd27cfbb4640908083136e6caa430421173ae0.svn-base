...
public class DTLenovoScrewMatrixExcelWriter implements IMatrixBOMExportWriter{
	...
	public String writerBOM(TCComponentItemRevision matrixItemRevision, List<MatrixBOMExportBean> beanList,ArrayList<String> varList) throws Exception {
		...
		String innerResPath = "com/foxconn/electronics/matrixbom/export/template/DT Lenovo Screw Matrix Change List.xlsx";
		URL url = this.getClass().getClassLoader().getResource(innerResPath);			
		InputStream in = url.openStream();
		
		String dir = CommonTools.getFilePath("DT Lenovo Screw Matrix");
...
//		MatrixBOMExportService.setValueAndStyle(writer,9+i, headerRowNum, "Remark",headStyle);
		...
		for(;i<beanList.size();i++) {
			...
			List<MatrixBOMExportBean> packList = bean.packList;
			if(CommonTools.isNotEmpty(packList)) {
				intpack = packList.size();
				writer.merge(row, row+intpack, 0, 0, lastNo, false);
				writer.merge(row, row+intpack, 3, 3, "", false);
				insertImg(writer.getWorkbook(), writer.getSheet(), bean.specPic, 3,row,3,row+intpack+1); 
				writer.setRowHeight(row, 80/(packList.size()+1));
...
//			hhpn = bean.hhpn;
			...
			if(CommonTools.isNotEmpty(substitutes)) {
				for (MatrixBOMExportBean substitute : substitutes) {
			...
			if(CommonTools.isNotEmpty(packList)) {
				...
				for (MatrixBOMExportBean pack:packList) {

					row++;
					
					writer.writeCellValue(1,row,pack.category);
					writer.writeCellValue(2,row,pack.description);
					writer.writeCellValue(4,row,pack.factory);
					writer.writeCellValue(5,row,pack.lenovoPN);
					writer.writeCellValue(6,row,pack.hhpn);
					writer.writeCellValue(7,row,pack.vendor);
					writer.writeCellValue(8,row,pack.torqueIn);
					writer.writeCellValue(9,row,pack.torqueOut);
					
					size = pack.variablesList.size();
					j = 1;
					for(;j<=size;j++) {
						writer.writeCellValue(9+j,row,pack.variablesList.get(j-1).getValue());
					}
//					writer.writeCellValue(9+j,row,bean.remark);
					writer.setRowHeight(row, 80/(packList.size()+1));
					
					hhpn = pack.hhpn;
					List<MatrixBOMExportBean> packSubstitutes = pack.substitutes;
					if(CommonTools.isNotEmpty(packSubstitutes)) {
						for (MatrixBOMExportBean packSubstitute : packSubstitutes) {
...
//		writer.getWorkbook().removeSheetAt(3);
	...
	public String writerDCN(TCComponentItemRevision matrixItemRevision,String dcnNo, List<MatrixBOMExportBean> list) throws Exception {
		...
		for(int i=0;i<list.size();i++) {
			...
			setValueAndStyle(writer, 12, row, bean.torqueIn, textCellStyle);
			setValueAndStyle(writer, 13, row, bean.torqueOut, textCellStyle);
	...
	public String createDCNwriterDCN(TCComponentItemRevision matrixItemRevision, String dcnNo,
		...
		String dir = CommonTools.getFilePath("DT Lenovo Screw Matrix_ChangeList");
		CommonTools.deletefile(dir); // 删除文件夹下面的所有文件
		String excelName = objectName.replace(" ","_") + "_" + revName + ".xlsx";
	...
	public MatrixBOMExportBean assembleMatixChangeBean(TCComponentBOMLine impactBOMLine,TCComponentBOMLine sulotionBomLine,String changeType) throws Exception {
		...
		e.torqueIn = mergeProperties(impactBOMLine, sulotionBomLine, "bl_occ_d9_TorqueIn");
		e.torqueOut = mergeProperties(impactBOMLine, sulotionBomLine, "bl_occ_d9_TorqueOut");
		return e;
	}
	...
	public List<MatrixBOMExportBean> getBeanList(MatrixBOMService matrixBOMService,String uid,ArrayList<String> varList) throws Exception {
		...
		for(ProductLineBOMBean productBean : productList) {
			...
			boolean isnotpack = true;
			if(CommonTools.isNotEmpty(list)) {
				
				String hhpn = assembleMatixBOMBean.hhpn;
				for (MatrixBOMExportBean exportBean : list) {
					String hhpn2 = exportBean.hhpn;
					if(hhpn.equals(hhpn2)) {
						isnotpack = false;
						
						List<MatrixBOMExportBean> packList = exportBean.packList;
						if(CommonTools.isEmpty(packList)) 
							packList = new ArrayList<MatrixBOMExportBean>();
						
						packList.add(assembleMatixBOMBean);
						exportBean.packList = packList;
					}
				}
			} 
