...
public class PartPropertyBean extends AbstractPropertyBean<Object> {
    ...
    protected static Text partText;

    public PartPropertyBean(Control paramControl) {
        super(paramControl);
    }

    public PartPropertyBean(FormToolkit paramFormToolkit, Composite paramComposite, boolean paramBoolean,
                            Map<?, ?> paramMap) {
        this.savable = true;
    ...
    public void dispose() {
        ...
        if (tcProperty != null) {
            try {
                ...
                if (!groupFullName.contains("Monitor.D_Group") && !groupFullName.contains("Printer.D_Group") && ItemTypeName.MYLARREVISION.equals(com.getType())) {
                    String objectName = Utils.getMylarName();
                    com.setProperty("object_name", objectName);
                    ((TCComponentItemRevision) com).getItem().setProperty("object_name", objectName);
                } else if (!groupFullName.contains("Monitor.D_Group") && !groupFullName.contains("Printer.D_Group") && ItemTypeName.LABELREVISION.equals(com.getType())) {
    ...
    public void load(TCPropertyDescriptor paramTCPropertyDescriptor) throws Exception {
        ...
        if (!Utils.isNull(defaultValue)) partText.setText(defaultValue);
    }
