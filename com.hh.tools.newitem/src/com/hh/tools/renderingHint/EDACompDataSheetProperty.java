...
public class EDACompDataSheetProperty extends JPanel implements InterfacePropertyComponent,
    ...
    public EDACompDataSheetProperty() {
        super();
        loadPropertyPanel();
    }
        ...
        button.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent arg0) {
                ...
                new Thread(new Runnable() {
                    public void run() {
                        new EDACompDatasetDialog("DataSheet Dataset", EDACompDataSheetProperty.this);
                    }
                }).start();
            }
        });
...
//			System.out.println("relationPath=====" + relationPath);
                String relationFileName = uploadRelationFile.getName();
//			System.out.println("relationFileName=====" + relationFileName);
                String datasetTypeName = datastTypeUtil.getDatasetType(relationFileName);
//			System.out.println("datasetTypeName=====" + datasetTypeName);
                String dstDefintionType = datastTypeUtil.getDatasetDefinitionType(datasetTypeName);
//			System.out.println("dstDefintionType=====" + dstDefintionType);
                // 创建数据集
                relationFileName = paramTCComponent.getProperty("fx8_Mfr") + "_"
                        + paramTCComponent.getProperty("fx8_MfrPN") + "."
                        + datastTypeUtil.getFileSuffix(relationFileName);
...
//			System.out.println("dataSheetDatasetComp===========" + dataSheetDatasetComp);
            ...
            if (null != dataSheetDatasetComp) {
                ...
                paramTCComponent.add(RelationName.DATASHEET, dataSheetDatasetComp);
            }
        } catch (TCException e) {
    ...
    public void clearDateSheet(TCComponent paramTCComponent) {
        try {
            TCComponent[] refComp = paramTCComponent.getRelatedComponents(RelationName.DATASHEET);
//			System.out.println("===========TCComponent[]=========length=" + refComp.length);
            if (null != refComp && refComp.length > 0) {
                paramTCComponent.remove(RelationName.DATASHEET, refComp);
            }
        } catch (TCException e) {
            e.printStackTrace();
        }
...
//		System.out.println("datesheet================load(TCComponent paramTCComponent)");
        if (this.property != null) {
            TCProperty localTCProperty = paramTCComponent.getTCProperty(this.property);
            TCComponent datasetComponent = paramTCComponent.getRelatedComponent(RelationName.DATASHEET);
