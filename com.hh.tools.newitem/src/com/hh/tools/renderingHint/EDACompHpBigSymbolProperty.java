...
public class EDACompHpBigSymbolProperty extends JPanel
    ...
    private String defalutDatasetFolder = "BigSymbols/HP";
    ...
    public EDACompHpBigSymbolProperty() {
        super();
        loadPropertyPanel();
    }
        ...
        button.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent arg0) {
                ...
                new Thread(new Runnable() {
                    public void run() {
                        new EDACompDatasetDialog("HP Big Symbols Dataset", EDACompHpBigSymbolProperty.this);
                    }
                }).start();
            }
        });
    ...
    public void setDateSheet(TCComponent paramTCComponent) {
        ...
        try {
            ...
            if (dataSheetDatasetComp != null) {
                ...
                paramTCComponent.add(RelationName.BIGSYMBOLREL, dataSheetDatasetComp);
    ...
    private void setSchematicPart(TCComponent paramTCComponent, TCComponentDataset dataSheetDatasetComp)
            throws TCException {
        String symbolName = paramTCComponent.getProperty(RelationName.SYMBOL);
    ...
    public void clearDateSheet(TCComponent paramTCComponent) {
        try {
            TCComponent[] refComp = paramTCComponent.getRelatedComponents(RelationName.BIGSYMBOLREL);
            if (null != refComp && refComp.length > 0) {
                for (int i = 0; i < refComp.length; i++) {
                    TCComponent itemRefComp = refComp[i];
                    paramTCComponent.remove(RelationName.BIGSYMBOLREL, itemRefComp);
                }
            }
        } catch (TCException e) {
            e.printStackTrace();
