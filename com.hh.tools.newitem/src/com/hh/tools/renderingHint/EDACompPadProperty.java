...
public class EDACompPadProperty extends JPanel implements InterfacePropertyComponent,
...
//				System.out.println("footprint otherFolderPath=====" + otherFolderPath);
                ...
                new Thread(new Runnable() {
                    public void run() {
                        new EDACompDatasetDialog("Pad Dataset", EDACompPadProperty.this);
                    }
                }).start();
            }
        });
    ...
    private static void setDatasetPath() {
        ...
        if (StringUtils.isNotEmpty(categoryVal) && StringUtils.isNotEmpty(footprintCategoryVal)) {
            if ("Special".equalsIgnoreCase(categoryVal)) {
                otherFolderPath = "Special/PAD";
            } else {
                otherFolderPath = "Standard/PAD";
            }
        }
    }
    ...
    public void setDateSheet(TCComponent paramTCComponent) throws TCException {
        ...
        if (null != uploadRelationFile) {
            ...
            // 创建数据集
            dataSheetDatasetComp = CreateObject.createDataSet(paramTCComponent.getSession(), relationPath, "FX8_PAD",
                    relationFileName, "FX8_PAD");
        ...
        if (null != relationDataset) {
            ...
            for (int i = 0; i < tcfiles.length; i++) {
                ...
                dataSheetDatasetComp = CreateObject.createDataSet(paramTCComponent.getSession(),
                        tempfile.getAbsolutePath(), "FX8_PAD", filename, "FX8_PAD");
            }
        }
        ...
        if (null != dataSheetDatasetComp) {
            try {
                ...
                paramTCComponent.add(RelationName.PAD, dataSheetDatasetComp);
            } catch (TCException e) {
    ...
    public void clearDateSheet(TCComponent paramTCComponent) {
        try {
            TCComponent[] refComp = paramTCComponent.getRelatedComponents(RelationName.PAD);
//			System.out.println("===========TCComponent[]=========length=" + refComp.length);
            if (null != refComp && refComp.length > 0) {
                for (int i = 0; i < refComp.length; i++) {
                    ...
                    paramTCComponent.remove(RelationName.PAD, itemRefComp);
                }
            }
        } catch (TCException e) {
            e.printStackTrace();
...
//		System.out.println("datesheet================load(TCComponent paramTCComponent)");
        if (this.property != null) {
            TCProperty localTCProperty = paramTCComponent.getTCProperty(this.property);
            TCComponent datasetComponent = paramTCComponent.getRelatedComponent(RelationName.PAD);
