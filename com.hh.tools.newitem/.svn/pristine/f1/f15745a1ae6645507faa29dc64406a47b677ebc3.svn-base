...
public class UploadPadPropertyBean extends AbstractPropertyBean<Object> {
    ...
    public UploadPadPropertyBean(Control paramControl) {
        super(paramControl);
    }

    public UploadPadPropertyBean(FormToolkit paramFormToolkit, Composite paramComposite, boolean paramBoolean,
                                 Map<?, ?> paramMap) {
        this.savable = true;
    ...
    private void doSave() {
        ...
        try {
            ...
            if (!Utils.isNull(path) && (path.endsWith("zip"))) {
                ...
                String padType = fileName.substring(0, fileName.lastIndexOf("."));
                if (padType.toLowerCase().endsWith("zip")) {
                    tcComponent = CreateObject.createDataSet(Utils.getTCSession(), path, "Zip", fileName, "ZIPFILE");
                } else if (padType.toLowerCase().endsWith("7z")) {
                    tcComponent = CreateObject.createDataSet(Utils.getTCSession(), path, "FX8_7Z", fileName, "FX8_7ZDst");
                } else if (padType.toLowerCase().endsWith("rar")) {
                    tcComponent = CreateObject.createDataSet(Utils.getTCSession(), path, "FX8_RAR", fileName, "FX8_RARDst");
                }
                com.add(RelationName.PAD, tcComponent);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
