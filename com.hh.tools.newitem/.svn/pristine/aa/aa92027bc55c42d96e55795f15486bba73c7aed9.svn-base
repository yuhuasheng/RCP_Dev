...
public class ProgramCustomerLovPropertyBean extends AbstractPropertyBean<Object> {
    ...
    public ProgramCustomerLovPropertyBean() {
        super();
        // TODO Auto-generated constructor stub
        System.out.println("Customer 1");
    }

    public ProgramCustomerLovPropertyBean(Control paramControl) {
        super(paramControl);
        // TODO Auto-generated constructor stub
    ...
    public ProgramCustomerLovPropertyBean(FormToolkit paramFormToolkit,
        ...
        textField.addModifyListener(new ModifyListener() {
            ...
            public void modifyText(ModifyEvent paramModifyEvent) {
                ...
                if (propLov.getSelectedItem() != null) {
                    if (ProcTypeLovPropertyBean.propLov != null) {
                        String procType = ProcTypeLovPropertyBean.getText();
                        if ("HP".equalsIgnoreCase(getText()) && "L10".equals(procType)) {
    ...
    public Object getEditableValue() {
        ...
        mandatory = false;
        savable = false;
        System.out.println("Customer getEditableValue == "
    ...
    public TCProperty getPropertyToSave(TCProperty arg0) throws Exception {
        // TODO Auto-generated method stub
        System.out.println("Customer getPropertyToSave");
        return arg0;
    }
    ...
    public boolean isPropertyModified(TCProperty arg0) throws Exception {
        // TODO Auto-generated method stub
        System.out.println("Customer isPropertyModified 11111");
        return isModify;
    }
    ...
    public void load(TCProperty arg0) throws Exception {
        // TODO Auto-generated method stub
        System.out.println("Customer load 1");
        // super.load(arg0);
...
//				if(ItemTypeName.DIAGREVISION.equals(component.getType())){
//					String procType = ProcTypeLovPropertyBean.getText();
//					if(!Utils.isNull(procType)){
//						component.getTCProperty("fx8_ProcType").setStringValueArray(new String[]{procType});
//					}					
//				}
                TCComponentItemRevision itemRev = (TCComponentItemRevision) component;
                String projectStr = PlatformLovPropertyBean.getPrjName();
                List<String> list = PlatformLovPropertyBean.getSelectedProgramList();
                if (list.size() == 1) {
                    try {
                        ...
                        if (!Utils.isNull(projectStr)) {
                            String program = PlatformLovPropertyBean.getSelectedProgramList().get(0);
                            itemRev.getTCProperty("fx8_ProgramName").setStringValueArray(new String[]{program});
                            itemRev.getTCProperty("fx8_PlateformName").setStringValueArray(PlatformLovPropertyBean.getSelectedPlatform());
                            String projectName = PlatformLovPropertyBean.getPrjName();
                            String[] newKeys = new String[]{Utils.getTextValue("ProjectName")};
                            String[] newValues = new String[]{projectName};
                            List<InterfaceAIFComponent> newprojectList = Utils.search("__FX_FindProject", newKeys, newValues);
                            if (newprojectList != null && newprojectList.size() > 0) {
                                TCComponentProject project = (TCComponentProject) newprojectList.get(0);
                                project.assignToProject(new TCComponent[]{itemRev.getItem()});

                            }
                            if (ItemTypeName.EDASCHEMREVISION.equals(itemRev.getType())) {
                                String phaseStr = ProgramPhaseLovPropertyBean.getText();
                                StringBuffer buffer = new StringBuffer();
                                buffer.append(Utils.getPlatformName());
                                if (!Utils.isNull(phaseStr)) {
                                    buffer.append("_" + phaseStr + "_SCH");
                                }
                                String objectName = buffer.toString();
                                component.setProperty("object_name", objectName);
                                ((TCComponentItemRevision) component).getItem().setProperty("object_name", objectName);
                            }
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
    ...
    public AIFPropertyDataBean getDataBeanViewModel() {
        // TODO Auto-generated method stub
        // System.out.println("Customer getDataBeanViewModel");
...
//			propLov.removeAllItems();	
            ...
            if (project != null) {
                try {
                    Utils.byPass(true);
                    AIFComponentContext[] parentContext = project.whereReferenced();
                    if (parentContext != null && parentContext.length > 0) {
                        for (int i = 0; i < parentContext.length; i++) {
                            ...
                            if ("Prg0ProgramPlan".equals(component.getType())) {
                                String customerValue = component.getProperty("prg0Customer");
                                System.out.println("projectCustomerValue==" + customerValue);
                                propLov.addItem(customerValue);
                                propLov.setSelectedItem(customerValue);
                            }
                        }
                    }
                    Utils.byPass(false);
