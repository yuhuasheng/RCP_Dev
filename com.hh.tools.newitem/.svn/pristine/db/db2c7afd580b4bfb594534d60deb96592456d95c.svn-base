...
public class UploadMDDPropertyBean extends AbstractPropertyBean<Object> {
    ...
    public UploadMDDPropertyBean(Control paramControl) {
        super(paramControl);
    }

    public UploadMDDPropertyBean(FormToolkit paramFormToolkit, Composite paramComposite, boolean paramBoolean,
                                 Map<?, ?> paramMap) {
        this.savable = true;
...
//		fileNameText.addModifyListener(modifyListener);
    ...
    private void doSave() {
        System.out.println("saveMDD");
        try {
            TCComponent mddDs = null;
            String mddPath = getText();
            if (!Utils.isNull(mddPath) && (mddPath.endsWith("xlsx") || mddPath.endsWith("xls"))) {
            	System.out.println("±£´æMDD");
                ...
                com.add(RelationName.MDDREL, mddDs);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//		TCComponent dataset = com.getRelatedComponent(RelationName.MDDREL);
//		if(dataset!=null){
