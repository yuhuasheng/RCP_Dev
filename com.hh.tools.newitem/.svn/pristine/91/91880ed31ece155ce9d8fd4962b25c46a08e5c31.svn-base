...
public class EDACompDellSymbolProperty extends JPanel implements InterfacePropertyComponent,
    ...
    public EDACompDellSymbolProperty() {
        super();
        loadPropertyPanel();
    }
        ...
        button.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent arg0) {
                ...
                new Thread(new Runnable() {
                    public void run() {
                        new EDACompDatasetDialog("Dell Symbols Dataset", EDACompDellSymbolProperty.this);
                    }
                }).start();
            }
        });
    ...
    private static void setDatasetPath() {
        ...
        String partTypeVal = EDACompPartTypeProperty.getValue();
    ...
    public TCProperty getPropertyToSave(TCComponent paramTCComponent) throws Exception {
        if (this.property != null) {
            TCProperty tcProperty = paramTCComponent.getTCProperty(this.property);
            if (uploadRelationFile != null) {
                tcProperty.setStringValue(uploadRelationFile.getName());
                setDateSheet(paramTCComponent);
                return tcProperty;
            }
            if (relationDataset != null) {
                tcProperty.setStringValue(relationDataset.toDisplayString());
                setDateSheet(paramTCComponent);
                return tcProperty;
            }
        }
    ...
    public void setDateSheet(TCComponent paramTCComponent) {
        ...
        try {
            ...
            if (dataSheetDatasetComp != null) {
                ...
                paramTCComponent.add(RelationName.DELLSYMBOLREL, dataSheetDatasetComp);
                String symbolName = dataSheetDatasetComp.toDisplayString();
                paramTCComponent.setProperty("fx8_DellSchematicPart", symbolName.substring(0, symbolName.lastIndexOf(".")));
            }
        } catch (TCException e) {
    ...
    public void clearDateSheet(TCComponent paramTCComponent) {
        try {
            TCComponent[] refComp = paramTCComponent.getRelatedComponents(RelationName.DELLSYMBOLREL);
            if (null != refComp && refComp.length > 0) {
                for (int i = 0; i < refComp.length; i++) {
                    TCComponent itemRefComp = refComp[i];
                    paramTCComponent.remove(RelationName.DELLSYMBOLREL, itemRefComp);
                }
            }
        } catch (TCException e) {
            e.printStackTrace();
...
//		System.out.println("datesheet================load(TCComponent paramTCComponent)");
        if (this.property != null) {
            TCProperty localTCProperty = paramTCComponent.getTCProperty(this.property);
            TCComponent datasetComponent = paramTCComponent.getRelatedComponent(RelationName.DELLSYMBOLREL);
