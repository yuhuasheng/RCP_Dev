...
public class EDACompSymbolProperty extends JPanel implements InterfacePropertyComponent,
        ...
        button.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent arg0) {
                ...
                new Thread(new Runnable() {
                    public void run() {
                        new EDACompDatasetDialog("HP Symbols Dataset", EDACompSymbolProperty.this);
                    }
                }).start();
            }
        });
    ...
    public void setDateSheet(TCComponent paramTCComponent) {
        ...
        try {
            ...
            if (dataSheetDatasetComp != null) {
                ...
                paramTCComponent.add(RelationName.SYMBOLREL, dataSheetDatasetComp);
    ...
    public void clearDateSheet(TCComponent paramTCComponent) {
        try {
            TCComponent[] refComp = paramTCComponent.getRelatedComponents(RelationName.SYMBOLREL);
            if (null != refComp && refComp.length > 0) {
                for (int i = 0; i < refComp.length; i++) {
                    TCComponent itemRefComp = refComp[i];
                    paramTCComponent.remove(RelationName.SYMBOLREL, itemRefComp);
                }
            }
        } catch (TCException e) {
            e.printStackTrace();
...
//		System.out.println("datesheet================load(TCComponent paramTCComponent)");
        if (this.property != null) {
            TCProperty localTCProperty = paramTCComponent.getTCProperty(this.property);
            TCComponent datasetComponent = paramTCComponent.getRelatedComponent(RelationName.SYMBOLREL);
