...
public class UploadFMDPropertyBean extends AbstractPropertyBean<Object> {
    ...
    public UploadFMDPropertyBean(Control paramControl) {
        super(paramControl);
    }

    public UploadFMDPropertyBean(FormToolkit paramFormToolkit, Composite paramComposite, boolean paramBoolean,
                                 Map<?, ?> paramMap) {
        this.savable = true;
...
//		fileNameText.addModifyListener(modifyListener);
    ...
    private void doSave() {
        ...
        try {
            TCComponent fmdDs = null;
            String fmdPath = getText();
            if (!Utils.isNull(fmdPath) && (fmdPath.endsWith("xlsx") || fmdPath.endsWith("xls"))) {
            	System.out.println("±£´æFMD");
                ...
                com.add(RelationName.FMDREL, fmdDs);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//		TCComponent dataset = com.getRelatedComponent(RelationName.FMDREL);
//		if(dataset!=null){
