...
import com.foxconn.plm.tcapi.utils.dataset.DsCreator;
...
public class BOMCommonOperate {
	  ...
	  Logger log=LogManager.getLogger(BOMCommonOperate.class);
	 ...
	 * @throws Exception
	 */
	public boolean isBom(StructureManagementService structureManagementService, StructureService strucService,
		...
		try {
			...
			String item_item_id = TCPublicUtils.getPropStr(dataManagementService, topLine, BOMLinePropConstant.BL_ITEM_ITEM_ID);
			System.out.println("==>> item_item_id: " + item_item_id);
			...
			if (bomWindows != null) {
//				BOMFacade.commonOperate.saveBOMWindow(structureManagementService, bomWindows[0]); // saveBOMWindow
				BOMFacade.commonOperate.closeBOMWindow(structureManagementService, bomWindows[0]); // close BOMWindow
			}
		}
	 ...
	 */
	public boolean isBom(DataManagementService dmService, ItemRevision itemRev) throws NotLoadedException {
		TCPublicUtils.refreshObject(dmService, itemRev);
		ModelObject[] modelObjects = TCPublicUtils.getPropModelObjectArray(dmService, itemRev, ItemRevPropConstant.PS_CHILDREN);
	...
	public void getChildBom(StructureService strucService, DataManagementService dataManagementService, BOMLine topLine,
		...
		for (ModelObject obj : children) {
			...
			if (itemRevList != null) {
				ModelObject modelObject = TCPublicUtils.getPropModelObject(dataManagementService, childBomLine,
						BOMLinePropConstant.BL_LINES_OBJECT);
				itemRevList.add(modelObject);
			}

			if (propertyList != null) {
				String refId = TCPublicUtils.getPropStr(dataManagementService, childBomLine,
						BOMLinePropConstant.BL_REF_DESIGNATOR);
				if (CommonTools.isNotEmpty(refId)) {
					propertyList.add(refId);
				}
			}
			boolean bIsBomLinePacked = TCPublicUtils.getPropBoolean(dataManagementService, childBomLine,
					BOMLinePropConstant.BL_IS_PACKED);
			...
			try {
				objects = TCPublicUtils.getPropModelObjectArray(dataManagementService, childBomLine,
						BOMLinePropConstant.BL_ALL_CHILD_LINES);
				if (list != null) {
	 ...
	 * @return
	 */
	public List openBOMWindow(StructureManagementService structureManagementService,
			DataManagementService dataManagementService, ItemRevision itemRevision) {
...
//			createBOMWindowsInfo[0].bomView =
	 ...
	 * @return
	 */
	public List openBOMWindowNew(StructureManagementService structureManagementService, ItemRevision itemRevision,
			PSBOMViewRevision psbomViewRevision) {
		List bomWindowParentLine = new ArrayList(2);
		try {
			CreateBOMWindowsInfo[] createBOMWindowsInfo = new CreateBOMWindowsInfo[1];
			createBOMWindowsInfo[0] = new CreateBOMWindowsInfo();
