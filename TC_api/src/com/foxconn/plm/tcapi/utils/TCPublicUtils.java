...
public class TCPublicUtils {
	...
	private static Logger log = LogManager.getLogger(TCPublicUtils.class);

	private static ICCTBOMLine icctbomLine;
	...
	public static void getProperty(DataManagementService dmService, ModelObject object, String propName) {
		...
		if (data.sizeOfPartialErrors() > 0) {
			for (int i = 0; i < data.sizeOfPartialErrors(); i++) {
				log.warn("【WARN】 warn info: " + Arrays.toString(data.getPartialError(i).getMessages()));
			}
		}
	}
	 ...
	 * @throws NotLoadedException
	 */
	public static Boolean getPropBoolean(DataManagementService dmService, ModelObject object, String propName)
			throws NotLoadedException {
		getProperty(dmService, object, propName);
	 ...
	 * @param properties
	 */
	public static void setDefaultLoadProperty(com.teamcenter.services.strong.core.SessionService sessionService,
			String objectType, String[] properties) {
...
//				anamevaluestruct1[j].values = new String[] {"0"};
	 ...
	 * @throws NotLoadedException
	 */
	public static Map<String, String> getEmail(SavedQueryService savedQueryService,
			DataManagementService datamanagementservice, List<String> list) {
		...
		list.forEach(userId -> {
			try {
				...
				User user = null;
				Person person = null;
				userObjects = executequery(savedQueryService, datamanagementservice,
						TCSearchEnum.__WEB_FIND_USER.queryName(), TCSearchEnum.__WEB_FIND_USER.queryParams(),
						new String[] { userId });
	 ...
	 * @return
	 */
	public static List<SPASUser> getSpasEmail(String requestPath, String ruleName, String userId) {
		List<SPASUser> actualUserLst = null;
		HashMap<String, String> map = new HashMap<String, String>();
		map.put("requestPath", requestPath);
		map.put("ruleName", ruleName);
		map.put("ruleName", userId);
		String result = HttpUtil.httpPost(map);
		if (CommonTools.isNotEmpty(result)) {
			Gson gson = new Gson();
			actualUserLst = gson.fromJson(result, new TypeToken<List<SPASUser>>() {
			}.getType());
			if (CommonTools.isNotEmpty(actualUserLst)) {
				checkActualUserInfo(actualUserLst);
			}
		} else {
			System.out.println("查询SPAS账号邮箱失败");
		}
	...
	private static void checkActualUserInfo(List<SPASUser> actualUserLst) {
		actualUserLst.removeIf(
				obj -> CommonTools.isEmpty(obj.getNotes().trim()) || CommonTools.isEmpty(obj.getName().trim())); // 移除邮箱和名字为空的记录
	}
	 ...
	 * @return
	 */
	public static ModelObject[] executequery(SavedQueryService queryService,
			DataManagementService datamanagementservice, String queryname, String[] entries, String[] values) {
	...
	public static List<String> getTcPreference(Connection connection, String preferName, String site) {
		try {
			com.teamcenter.services.strong.core.SessionService sessionService = com.teamcenter.services.strong.core.SessionService.getService(connection);
	 ...
	 */
	public static List<String> getTCPreferences(PreferenceManagementService preferService, String preferName) throws ServiceException {
		try {
			preferService.refreshPreferences();
			GetPreferencesResponse getpreferencesRes = preferService.getPreferences(new String[]{preferName}, false);
	        ...
	        if (CommonTools.isNotEmpty(temps)) { // 不为空
	 ...
	 * @throws ServiceException
	 */
	public static ModelObject checkout(DataManagementService dataManagementService, Connection connection,
			ModelObject object) throws NotLoadedException, ServiceException {
	 ...
	 * @return
	 */
	public static String getFileServerIp(Connection connection) {
		String fileServerIp = null;
		String def = null;
		try {
			String ip = InetAddress.getLocalHost().getHostAddress();
			System.out.print("ip==================" + ip);
			List<String> fs = getTcPreference(connection, TCPreferenceConstant.D9_FILESERVER_LIST, "site");
	...
	public static ModelObject[] getSubsLine(DataManagementService ds, Connection connection, BOMLine bomLine) {
		icctbomLine = new ICCTBOMLine(connection, "BOMLine", "TYPE::BOMLine::BOMLine::RuntimeBusinessObject");
	 ...
	 public static ModelObject[] loadObject(DataManagementService ds, String[] uids) throws IOException {
