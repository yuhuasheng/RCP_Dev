...
public class BOMModifier {
	 ...
	 Logger log=LogManager.getLogger(BOMModifier.class);
	 ...
	 * @throws Exception
	 */
	public void packOrUnpack(StructureService strucService, DataManagementService dmService, BOMLine bomLine, int level) throws Exception {
		boolean isSub = TCPublicUtils.getPropBoolean(dmService, bomLine, BOMLinePropConstant.FND0BL_IS_SUBSTITUTE);
		if (isSub) {
			return;
		}
		boolean is_packed =  bomLine.get_bl_is_packed();
		if (is_packed) { // 判断是否为打包状态
			...
			if (response.sizeOfPartialErrors() == 0) {
				log.info("unpacking successfull ");
			} else {
				throw new Exception("unpacking failure");
			}
		}
		
		
		ModelObject[] children = TCPublicUtils.getPropModelObjectArray(dmService, bomLine, BOMLinePropConstant.BL_ALL_CHILD_LINES);		
		if (CommonTools.isNotEmpty(children)) {
			Stream.of(children).parallel().forEach(obj -> {
				try {
					BOMLine childBomLine = (BOMLine) obj;
					packOrUnpack(strucService, dmService, childBomLine, level);
				} catch (Exception e) {
