...
//  private static final Logger logger = Logger.getLogger(GenericUserSelectionPanel.class);
	...
	public UserGenericUserSelectionPanel(Frame paramFrame, TCSession paramTCSession) {
		this(paramFrame, paramTCSession, null);
	}

	public UserGenericUserSelectionPanel(TCSession paramTCSession) {
		this(null, paramTCSession);
	}

	public UserGenericUserSelectionPanel(Frame paramFrame, TCSession paramTCSession, TCComponentGroup paramTCComponentGroup) {
		super(true);
...
//			Logger.getLogger(UserGenericUserSelectionPanel.class).error(tCException.getLocalizedMessage(), tCException);
	...
	public void validateButtons() {
		if (this.signOffType != null && this.signOffType.equalsIgnoreCase("TCComponentProfile") && this.selectedProfileChildCount >= this.signOffsRequired) {
	...
	public void disableButtons(String paramString) {
		...
		if (this.signOffType != null && (this.signOffType.equals("TCComponentSignoff") || this.signOffType.equalsIgnoreCase("TCComponentGroupMember"))) {
...
//                      logger.error(exception.getClass().getName(), exception);
	...
	private void searchOrgTree(String paramString1, String paramString2, String paramString3) {
		this.orgTreePanel.performSearchInContext(this.selectedProfileGrpComp, this.selectedProfileRoleComp, this.subGroupsAllowed);
	}

	public String checkSignOffsSelectedAreValid() {
		OrgObject[] arrayOfOrgObject = getSelectedObjects();
		return (arrayOfOrgObject == null || arrayOfOrgObject.length == 0) ? "" : getReasonForInvalidSelection(arrayOfOrgObject);
	}
...
//					logger.error(tCException.getClass().getName(), tCException);
	...
	public OrgObject[] getSelectedObjects() {
		...
		for (int b2 = 0; arrayOfTCTreeNode != null && b2 < arrayOfTCTreeNode.length; b2++) {
			OrgObject orgObject = this.orgUserSelectionPanel.getOrgTreePanel().getOrgTree().getOrgObject(arrayOfTCTreeNode[b2]);
	...
	public TCComponentProject[] getValidProjects(TCComponent[] paramArrayOfTCComponent) {
		...
		try {
			...
			arrayOfTCComponentProject = (TCComponentProject[]) arrayList3.toArray(new TCComponentProject[arrayList3.size()]);
...
//			logger.error(tCException.getClass().getName(), tCException);
	...
	public void setTargetObjects(ArrayList paramArrayList) {
		...
		if (!this.m_projectTabRemoved && (bool || this.m_showPanels.equals("project_default") || this.m_showPanels.equals("project_only")))
			updateTab(1);
	}
