...
import com.teamcenter.rac.workflow.commands.adhoc.SignoffTreeNode;
...
//  private static final Logger logger = Logger.getLogger(SignoffEditPanel.class);
	...
	public UserSignoffEditPanel(TCSession paramTCSession) {
...
//      	logger.error("Exception", tCException);
		...
		this.genericUserSelectionPanel = new UserGenericUserSelectionPanel(this.session);
		...
		TreeSelectionListener treeSelectionListener = new TreeSelectionListener() {
			public void valueChanged(TreeSelectionEvent param1TreeSelectionEvent) {
				...
				if (str != null && !str.equals("")) {
					...
					if (jPanel instanceof AdhocSignoffsPanel) {
						AdhocSignoffsPanel adhocSignoffsPanel = (AdhocSignoffsPanel) jPanel;
						// recompile 20220323143000 : START
//						TCComponentTask tCComponentTask = adhocSignoffsPanel.task;
						TCComponentTask tCComponentTask = (TCComponentTask)getPropty(adhocSignoffsPanel, "task");
						// 20220323143000 : END
...
//							logger.error(exception.getClass().getName(), exception);
	...
	public void setComponent(SignoffTreeNode paramSignoffTreeNode, Object paramObject, TCComponentTask paramTCComponentTask) {
		...
		boolean bool = !(!(paramObject instanceof TCComponentAliasList) && paramObject != TCSignoffOriginType.ALIASLIST
				&& paramObject != TCSignoffOriginType.ADDRESSLIST);
...
//			logger.error("Exception", exception);
	...
	public void setNewComponent(TCTreeNode paramTCTreeNode, TCComponent paramTCComponent,
		...
		for (i = 0; i < vector.size(); i++) {
			...
			jRadioButton.setText(translate2Text(j));
			jRadioButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent param1ActionEvent) {
					if (j == 1 && UserSignoffEditPanel.this.isSignoffRequired()) {
						UserSignoffEditPanel.this.actionType = 4;
					} else if (j == 2 && UserSignoffEditPanel.this.isSignoffRequired()) {
						UserSignoffEditPanel.this.actionType = 5;
					} else {
						UserSignoffEditPanel.this.actionType = j;
					}
				}
			});
...
//			Logger.getLogger(SignoffEditPanel.class).error(tCException.getLocalizedMessage(), tCException);
	...
	private void setProjectTeamFilter(TCComponentGroup paramTCComponentGroup, TCComponentRole paramTCComponentRole, boolean paramBoolean) {
...
//			logger.error("Exception", exception);
	...
	// recompile 20220323143000 : START
	public Object getPropty(AdhocSignoffsPanel adhocSignoffsPanel, String property) {		
		
		try {
			Class clazz = Class.forName(adhocSignoffsPanel.getClass().getName());
			Field field;
			field = clazz.getDeclaredField(property);
			System.out.println(field);
			field.setAccessible(true);

			return field.get(adhocSignoffsPanel);
	...
	public void clearPubMailContent() {
//		this.genericUserSelectionPanel.publicMailPanel.initializePanel();
	}
	// 20220323143000 : START
